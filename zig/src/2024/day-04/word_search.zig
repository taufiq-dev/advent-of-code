...
pub fn part1(currRow: *u8, currCol: *u8, max_cols: *u8, grid: [][]u8) !void {
...
pub fn part2(currRow: *u8, currCol: *u8, max_cols: *u8, grid: [][]u8) !void {
    var xmasCount: u16 = 0;
    while (currRow.* < 140) : (currRow.* += 1) {
        currCol.* = 0;
        while (currCol.* < max_cols.*) : (currCol.* += 1) {
...
pub fn main() !void {
    ...
    try part2(&currRow, &currCol, &max_cols, grid);
}
